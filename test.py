...
for x in api:
    for coin in my_portfolio:
        if coin["sym"] == x["symbol"]:
            ...
            one_hr_change = Label(root, text="${0:.2f}".format(float(x["percent_change_1h"])), bg="silver",
                                  fg=red_green(float(x["percent_change_1h"])))
            one_hr_change.grid(row=row_count, column=5, sticky=N + S + E + W)

            tf_hr_change = Label(root, text="${0:.2f}".format(float(x["percent_change_24h"])), bg="white",
                                 fg=red_green(float(x["percent_change_24h"])))
            tf_hr_change.grid(row=row_count, column=6, sticky=N + S + E + W)

            seven_day_change = Label(root, text="${0:.2f}".format(float(x["percent_change_7d"])), bg="silver",
                                     fg=red_green(float(x["percent_change_7d"])))
            ...
            current_value = Label(root, text=" ${0:.2f}".format(float(current_value)), bg="white",
                                  fg=red_green(float(current_value)))
